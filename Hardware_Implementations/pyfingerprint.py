...
class PyFingerprint(object):
    ...
    def __readPacket(self):
        """
        Receives a full packet from the sensor, with buffer-aware reads.

        Returns:
            tuple: (packetType, packetPayload)
        
        Raises:
            Exception if the packet is malformed or checksum is invalid.
        """
        

        # Step 1: Wait until the minimal header is available (at least 9 bytes)
        while self.__serial.any() < 9:
            time.sleep(0.005)

        # Step 2: Read the first 9 bytes (fixed header size)
        header = self.__serial.read(9)
        if len(header) < 9:
            raise Exception("Timeout while reading packet header")

        # Step 3: Parse basic fields
        start_code = (header[0] << 8) | header[1]
        if start_code != FINGERPRINT_STARTCODE:
            raise Exception("The received packet does not begin with a valid header!")

        packetType = header[6]
        packetLength = (header[7] << 8) | header[8]

        # Step 4: Wait until full packet is available
        while self.__serial.any() < packetLength:
            time.sleep(0.005)

        # Step 5: Read the rest of the packet (payload + 2-byte checksum)
        payload_and_checksum = self.__serial.read(packetLength)
        if len(payload_and_checksum) != packetLength:
            raise Exception("Timeout while reading packet payload")

        # Step 6: Extract payload and checksum
        packetPayload = payload_and_checksum[:-2]
        receivedChecksum = (payload_and_checksum[-2] << 8) | payload_and_checksum[-1]

        # Step 7: Calculate expected checksum
        checksum = packetType + header[7] + header[8] + sum(packetPayload)

        if checksum != receivedChecksum:
            raise Exception("The received packet is corrupted (the checksum is wrong)!")

        return (packetType, list(packetPayload))
    ...
    def downloadImage(self, path=None):
        """
        Downloads the fingerprint image from the sensor and optionally saves it to a file.

        Args:
            path (str): Optional path to save the image as a raw binary file.

        Returns:
            The raw image bytes as a list.
        """
        # Step 1: Send the command to start image download
        packetPayload = (FINGERPRINT_DOWNLOADIMAGE,)
        self.__writePacket(FINGERPRINT_COMMANDPACKET, packetPayload)

        # self.getMaxPacketSize()

        # Step 2: Expect an ACK packet
        receivedPacket = self.__readPacket()
        receivedPacketType = receivedPacket[0]
        receivedPacketPayload = receivedPacket[1]

        if receivedPacketType != FINGERPRINT_ACKPACKET:
            raise Exception("Didn't receive ACK packet")

        if receivedPacketPayload[0] != FINGERPRINT_OK:
            raise Exception(f"Sensor returned error code: {hex(receivedPacketPayload[0])}")

        # Step 3: Read data packets
        image_data = bytearray()
        print("Reading image stream")
        while True:
            packetType, payload = self.__readPacket()

            if packetType not in (FINGERPRINT_DATAPACKET, FINGERPRINT_ENDDATAPACKET):
                raise Exception("Unexpected packet type received")

            image_data.extend(bytes(payload))
